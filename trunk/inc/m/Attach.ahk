...
Attach_(hCtrl, aDef, Msg, hParent){
	...
	if (hParent = "")  {						;Initialize controls 
		if !adrSetWindowPos
			adrSetWindowPos		:= DllCall("GetProcAddress", "uint", DllCall("GetModuleHandle", a() "str", "user32"), "str", "SetWindowPos")
			,adrWindowInfo		:= DllCall("GetProcAddress", "uint", DllCall("GetModuleHandle", a() "str", "user32"), "str", "GetWindowInfo")
